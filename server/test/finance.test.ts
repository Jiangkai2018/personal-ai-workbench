...
const WECHAT_ROWS = [
  ...
  ['2026-08-17 20:00:00', '转账', 'Lover (family)', '微信转账', '支出', 500.0, '零钱', '已转账', '4200003231wx02', '/', '转账备注'],
...
describe('账单解析器', () => {
  it('微信 xlsx：识别昵称、过滤中性/未成功、列映射正确', () => {
    const buf = buildWeChatXlsx('wechat-user', WECHAT_ROWS)
    const r = parseBillFile('微信支付账单流水文件.xlsx', buf)
    expect(r.source).toBe('wechat')
    expect(r.owner).toBe('wechat-user')
